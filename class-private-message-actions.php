...
class AE_Private_Message_Actions extends AE_PostAction
{
    ...
    public function init()
    {
        ...
        $this->add_action( 'wp_ajax_file_upload_chat_message', 'file_upload_chat_message');
        $this->add_action( 'wp_ajax_update_user_status', 'update_user_status' );
        // disable from 1.1.5
    ...
    public function ae_private_message_convert_bid($result) {
        ...
        $response = ae_private_message_created_a_conversation(array('bid_id'=>$result->ID));
        $html_message = '';
    ...
    ?>
            <button
                   ...
                   title=""
                    data-conversation="<?php if( isset($response['conversation_id']) ) echo $response['conversation_id'] ?>">
                <i class="fa fa-comment"></i><span><?php _e('Message', ET_DOMAIN) ?></span>
            </button>
    <?php
            $html_message = ob_get_clean();
    ...
    public function ae_private_message_convert($result){
        ...
        $result->conversation_author_online_status = '';
        if( $author ){
          ...
          $result->conversation_author_online_status = (function() use ($author){
            $time_elapsed = (intval(time() . "000") - intval(get_user_meta($author->ID, 'last_activity', true)))/1000;
            if ($time_elapsed < 300) {
              return 'online';
            } elseif ($time_elapsed < 900) {
              return 'away';
            }
          })();
        }
        ...
        if($result->is_conversation){
	        $last_conversation = get_posts(array(
        ...
        $result->post_time = get_post_time('G:i:s A', true, $result->ID);
    ...
    public function ae_private_message_notify_item( $content, $notify ){
        ...
        if($type == 'new_private_message'){
	        ...
	        $project = !empty($project) ? $project :  array();
            ...
            $message = sprintf(__('%s sent you a private message on the project %s', ET_DOMAIN),
                            '<strong>'.get_the_author_meta('display_name', $sender).'</strong>',
                            '<strong>'.get_the_title($project).'</strong>'
                        );
            $content .= '<a class="fre-notify-wrap" href="'.$message_link.'">
    ...
    public function ae_private_message_sync_conversation($request){
        ...
        if (!is_wp_error($result)) {
            ...
            if( $user_email && $is_create ) {
               ...
               $this->ae_private_message_email_notification( $user_email, $subject, $message, array( 'user_id'=> $receiver_id));
            ...
            if( $is_create ){
              ...
              **/
              ...
              $clone_reply['meta_input'] = array( 'is_conversation' => 0,'is_conversation_clone' => 1);
    ...
    public function file_upload_chat_message(){
		$file_name = $_FILES[ 'attachment_upload' ][ 'name' ];
        $upload_dir = "chat_files/".$file_name;
        $upload_path = wp_upload_dir()[ 'basedir' ] . "/$upload_dir";
        move_uploaded_file( $_FILES[ 'attachment_upload' ][ 'tmp_name' ], $upload_path );
        echo wp_upload_dir()['baseurl']."/$upload_dir";
        wp_die();
	}
    ...
    **/
    ...
    function update_user_status() {
      	//var_dump($this->newChatMessages());
        $new_messages = $this->newChatMessages(filter_input(INPUT_POST, 'chat_group_list'));

        $users = explode(',', $_POST['active_user']);
        if ( filter_input( INPUT_POST, 'interaction' ) === 'true' ) {
            $interacted = 'true';
        } else if ( filter_input( INPUT_POST, 'interaction' ) === 'false' ) {
            $interacted = 'false';
        } else {
            $interacted = 'false';
        }
        update_user_meta( get_current_user_id(), '__user_chat_last_active', date( 'y-m-d h:i:s' ) );
        update_user_meta( get_current_user_id(), '__user_chat_interaction', $interacted );

         $chat_partner = $users;
        $users_status = array();
        foreach ( $chat_partner as $value ) {

            $last_active_string = get_user_meta( $value, '__user_chat_last_active', true );
            $last_user_interaction = get_user_meta( $value, '__user_chat_interaction', true );

            $status_type = $this->return_status( $last_active_string, $last_user_interaction );

            $users_status[] = array( 'id' => $value, 'status_type' => $status_type, 'nickname' => get_user_meta($value, 'nickname', true) );
        }
        echo json_encode( array('status'=>$users_status, 'message' => $new_messages) );
        wp_die();
    }
  
  	function newChatMessages($list = "2063,1959,1676"){
      global $wpdb;
      $query = "select count(*) as counts, post_parent from f6ihrh4gzgposts where post_parent in ($list) group by post_parent";
      $result = $wpdb->get_results($query);
      return json_encode($result);
	}

    function return_status( $last_active_string, $last_user_interaction ) {
        if ( $last_active_string && $last_user_interaction ) {
            $last_active = date_create( $last_active_string );
            $current_time = date_create( date( 'y-m-d h:i:s' ) );
            if ( date_diff( $current_time, $last_active, true )->s > 10 ) {
                $status_type = 'I';
            } else if ( date_diff( $current_time, $last_active, true )->s <= 10 && $last_user_interaction == 'true' ) {
                $status_type = 'A';
            } else if ( date_diff( $current_time, $last_active, true )->s <= 10 && $last_user_interaction == 'false' ) {
                $status_type = 'U';
            }
        } else {
            $status_type = 'I';
